<script>
	...
	onMount(async () => {
		library = await spotify_fetch('https://api.spotify.com/v1/me/tracks?limit=20');

		if (library) {
			setTimeout(() => {
				const intersectionObserver = new IntersectionObserver(async (entries, observer) => {
					entries.forEach(async (entry) => {
						if (entry.isIntersecting && library.next) {
							const libraryUpdate = await spotify_fetch(library.next);

							library = {
								...libraryUpdate,
								items: [...library.items, ...libraryUpdate.items]
							};
						}
					});
				});

				intersectionObserver.observe(document.querySelector('.intersection-tracking-subject'));
			}, 1000);
		}
	});

	const handleLibraryPlay = async () => {
		$playerParams.deviceId
			? await spotify_fetch(
					`https://api.spotify.com/v1/me/player/play?device_id=${$playerParams.deviceId}`,
					'PUT',
					{
						context_uri: 'spotify:user:n9rpc026oyhuhvvcq4azzev5r:collection'
					}
			  )
			: null;
	};

	const handleLibraryPause = async () => {
		$playerParams.deviceId
			? await spotify_fetch(
...
<div class="library-page">
	{#if library}
		<div class="library">
			<img class="image" src="/images/red-heart.jpg" alt="music-library" />

			<div class="details">
				<h3 class="title">Liked tracks</h3>
				<h4 class="description">Your personal curated playlist</h4>
				<div class="sub-details">
					<div class="sub-detail">
						...
						<p class="sub-detail-text">{$user.display_name}</p>
					</div>
					<div class="sub-detail">
						<svg
							width="16"
						...
						<p class="sub-detail-text">{library.total} tracks</p>
					</div>
				</div>
			</div>

			<div class="actions">
				{#if $playerParams.contextURI === 'spotify:user:n9rpc026oyhuhvvcq4azzev5r:collection' && !$playerParams.isPaused}
					<button class="play-button" style="margin-right: 1rem;" on:click={handleLibraryPause}>
						<svg
							xmlns="http://www.w3.org/2000/svg"
							width="24"
							height="24"
				...
				{:else}
					<button class="play-button" style="margin-right: 1rem;" on:click={handleLibraryPlay}>
						<svg
							width="24"
							height="24"
							fill="currentColor"
		...
		<div class="track-list">
			...
			<div class="tracks">
				{#each library.items as item, index}
			...
			<div class="intersection-tracking-subject" />
		</div>
	{:else}
		<div class="loader">
...
<style>
	...
	.library {
		...
		position: relative;
		display: flex;
		background: linear-gradient(180deg, #fb9fa4, 50%, #00111c 90%);
	}
	...
	.library .actions .play-button:hover {
